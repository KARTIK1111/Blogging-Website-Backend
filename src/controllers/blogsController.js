...
const createBlog = async function (req, res) {
  try {
    ...
    if (data.isDeleted == true) {
      return res.status(400).send({ status: false, msg: "Bad request , Data is required " });
    } else {
...
const getBlog = async function (req, res) {
  try {
    ...
    const Blog = await blogsModel.find({isDeleted: false,isPublished: true,...queryParams});
...
const updateblog = async function (req, res) {
  try {
    ...
    const isPublishedtrue = await blogsModel.updateOne({ _id: blogId, isDeleted: false, isPublished: true },{ $set: { publishedAt: date }});

    //Successfully updated data
    return res.status(200).send({ status: true, msg: "Document successfuly Updated"  });
  }
   catch (err) {
    //its a exception handler part if your logic will not excute by any
    console.log("It seems an error", err.message);
...
const deleteBlogs = async function (req, res) {
  try {
    ...
    if (deleteTrue) {
      ...
      const deletedBlog = await blogsModel.updateMany({ _id: blogId, isDeleted: false  },
        { $set: { isDeleted: true, deletedAt: date ,isPublished:false }});
      
        if (deletedBlog.matchedCount == 0) {
        // it will count the number of deletetion
        return res.status(404).send({status: false,msg: "Document doesnt exist/Document Already Deleted"});
      }
      //check the condition, if matched then data deleted successfully
      if (deletedBlog) {
        return res.status(200).send({ status: true, msg: "Document successfuly Deleted" });
      }
    }
  } catch (err) {
    //its a exception handler part if your logic will not excute by any
...
const DeleteBlog = async function (req, res) {
 ...
 // is will update the isDeleted value false to true
    const Blog = await blogsModel.updateOne(
      { isDeleted: false, authorId:id, ...queryParams },{ $set: { isDeleted: true,isPublished:false}});
